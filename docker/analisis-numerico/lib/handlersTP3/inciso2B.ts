...
export async function handleInciso2B({
  ...
  try {
    ...
    const consignaB = await fetchTexto(`${API_BASE_URL}/gases/consigna-b`);
    const consignaTexto = `${consignaGral}\n\n${consignaB}`;

    // Dificultades
    const problemasTexto = await fetchTexto(
      `${API_BASE_URL}/gases/dificultad-b`
    );

    // Resultado
    const consolaTexto = await fetchTexto(`${API_BASE_URL}/gases/taylor-vdw`);
    ...
    for (const ruta of rutas) {
      const img = await fetchImagen(`${API_BASE_URL}/gases/${ruta}`);
