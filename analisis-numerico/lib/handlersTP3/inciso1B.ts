...
export async function handleInciso1B({
  ...
  try {
    ...
    const [consignaTexto, consolaTexto, problemasTexto] = await Promise.all([
      fetchTexto(`${API_BASE_URL}/tp3/raices/consigna-b`),
      fetchTexto(`${API_BASE_URL}/tp3/raices/inciso1b`),
      fetchTexto(`${API_BASE_URL}/tp3/raices/dificultad-b`),
    ]);
    ...
    const grafico = await fetchImagen(`${API_BASE_URL}/tp3/raices/grafico4`);
