...
export async function handleInciso1A({
  ...
  try {
    ...
    const [consignaTexto, taylorData, problemasTexto] = await Promise.all([
      fetchTexto(`${API_BASE_URL}/tp3/raices/consigna-a`),
      fetchJSON<{ salida: string }>(`${API_BASE_URL}/tp3/raices/taylor`),
      fetchTexto(`${API_BASE_URL}/tp3/raices/dificultad-a`),
    ]);
    ...
    for (let i = 1; i <= 2; i++) {
      try {
        const res = await fetch(`${API_BASE_URL}/tp3/raices/grafico${i}`, {
          method: "GET",
    ...
    for (let iter = 0; iter <= 2; iter++) {
      try {
        const res = await fetch(`${API_BASE_URL}/tp3/raices/grafico3/${iter}`, {
          method: "GET",
          headers: { Accept: "image/png" },
        });
        if (!res.ok) {
