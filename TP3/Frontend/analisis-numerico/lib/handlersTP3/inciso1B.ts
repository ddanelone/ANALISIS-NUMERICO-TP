...
export async function handleInciso1B({
  ...
  try {
    ...
    const [consignaTexto, consolaTexto, problemasTexto] = await Promise.all([
      ...
      fetchTexto(`${API_BASE_URL}/raices/inciso1b-multi`),
      fetchTexto(`${API_BASE_URL}/raices/dificultad-b-multi`),
    ]);
    ...
    const grafico = await fetchImagen(`${API_BASE_URL}/raices/grafico4-multi`);
