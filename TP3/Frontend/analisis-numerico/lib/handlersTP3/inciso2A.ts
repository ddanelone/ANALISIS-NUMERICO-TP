...
export async function handleInciso2A({
  ...
  try {
    ...
    const consignaGral = await fetchTexto(
      `${API_BASE_URL}/gases/consigna-gral`
    );
    const consignaA = await fetchTexto(`${API_BASE_URL}/gases/consigna-a`);
    const consignaTexto = `${consignaGral}\n\n${consignaA}`;

    // Explicación teórica
    const explicacionTexto = await fetchTexto(
      `${API_BASE_URL}/gases/explicacion-inciso-a`
    );

    // Dificultades encontradas
    const problemasTexto = await fetchTexto(
      `${API_BASE_URL}/gases/dificultad-a`
    );
...
`;
    ...
    const urls = [
      `${API_BASE_URL}/gases/grafico_comparativo_gral`,
      `${API_BASE_URL}/gases/grafico_comparativo_z`,
      `${API_BASE_URL}/gases/grafico_comparacion_volumenes`,
    ];
